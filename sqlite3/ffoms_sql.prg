// Справочники федерального фонда медицинского страхования РФ
...
function make_q015(db, source)
  ...
  if sqlite3_exec(db, 'DROP TABLE IF EXISTS q015') == SQLITE_OK
    OutStd(hb_eol() + 'DROP TABLE q015 - Ok' + hb_eol())
  endif
     
  if sqlite3_exec(db, cmdText) == SQLITE_OK
    OutStd( hb_eol() + 'CREATE TABLE q015 - Ok' + hb_eol() )
  else
    OutStd( hb_eol() + 'CREATE TABLE q015 - False' + hb_eol() )
    return nil
  endif
