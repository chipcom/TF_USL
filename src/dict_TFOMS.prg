...
Function work_SprUnit( source, destination )
  ...
  //
  // Письмо об изменении формата Справочников медуслуг и единиц объема 20-12-2017 04-18-34.pdf
  //
  // NAME - Наименование учетной единицы объема медицинской помощи
  // C_T - Способ определения выполненного объема по случаю для учетной единицы:
  // 1 ? выполненный объем рассчитывается как суммарное количество медуслуг, отнесенных к
  // одной учетной единице объема,
  // 2 ? выполненный объем по случаю принимается равным одной единице, вне зависимости от суммарного
